...
class VCanvas {
  ...
  line(x1, y1, x2, y2) {
    if (this._scalable) {
      var xx1 =
        (((x1 - this._scaleLeft) * this._canvas.width) / this._scaleWidth) *
        this._dir_x
      var xx2 =
        (((x2 - this._scaleLeft) * this._canvas.width) / this._scaleWidth) *
        this._dir_x
      var yy1 =
        (((y1 - this._scaleTop) * this._canvas.height) / this._scaleHeight) *
        this._dir_y
      var yy2 =
        (((y2 - this._scaleTop) * this._canvas.height) / this._scaleHeight) *
        this._dir_y
    } else {
      var xx1 = x1
      var xx2 = x2
  ...
  lineStart(x, y) {
    if (this._scalable) {
      var xx =
        (((x - this._scaleLeft) * this._canvas.width) / this._scaleWidth) *
        this._dir_x
      var yy =
        (((y - this._scaleTop) * this._canvas.height) / this._scaleHeight) *
        this._dir_y
    } else {
      var xx = x
      var yy = y
  ...
  rect(x1, y1, x2, y2) {
    // 長方形を描画
    var xx1 =
      (((x1 - this.scaleLeft) * this._canvas.width) / this._scaleWidth) *
      this._dir_x
    var yy1 =
      (((y1 - this.scaleTop) * this._canvas.height) / this._scaleHeight) *
      this._dir_y
    var xx2 =
      (((x2 - this.scaleLeft) * this._canvas.width) / this._scaleWidth) *
      this._dir_x
    var yy2 =
      (((y2 - this.scaleTop) * this._canvas.height) / this._scaleHeight) *
      this._dir_y
